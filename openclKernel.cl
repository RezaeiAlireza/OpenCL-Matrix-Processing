__kernel void myKernel(__global float *p_matrix, __global float *tmp_M) {
    ...
    if(first >= 2 && first < firsts-2 && second >= 2 && second < seconds-2) {
        ...
        tmp_M[currentIndex] = (seconds*0.02) +(native_sqrt(p_matrix[topLeftIndex] +
                            p_matrix[topRightIndex] +
                            p_matrix[bottomLeftIndex] +
                            p_matrix[bottomRightIndex]) +
                            native_sqrt(p_matrix[leftIndex] +
                            p_matrix[rightIndex] +
                            p_matrix[topIndex] +
                            p_matrix[bottomIndex]))*10;
    }
}

...
int main(int argc, char* argv[]){
    ...
    ser_matrix = (double *)malloc(sizeof(double)*size*size);
    tmp_MS = (double *)malloc(sizeof(double)*size*size);
    ...
    cout << "Serial version took "<<((double)(end-begin) / CLOCKS_PER_SEC)<<" seconds to execute"<<"\n";

    // Uncomment the code to save the results in a txt file

    // std::ofstream ofs_1("out_m_double.txt", std::ofstream::out);
    ...
    p_matrix = (double *)malloc(sizeof(double)*size*size);
    tmp_M = (double *)malloc(sizeof(double)*size*size);
    ...
    {   
        ...
        char *kernelSource;
        size_t kernelSize;
        int dim = size * size * sizeof(double);

        cout<<"Running parallel part:\n";
        kernelFile = fopen("originalKernelDouble.cl", "r");

        if (!kernelFile) {
            cout<<"No file named originalKernelDouble.cl was found\n";
            exit(-1);
        }
        ...
        for(int i=0;i<size;i++){
            for(int j=0;j<size;j++){
                if(p_matrix[i+j*size]-ser_matrix[i+j*size] > eps || p_matrix[i+j*size]-ser_matrix[i+j*size] < eps){
                    not_matched++;
                }
            }      
        }
        ...
        if(not_matched > 0){
            cout<<not_matched<<" of cells do not match\n";
        }else{
        ...
        // std::ofstream ofs_2("out_p_double.txt", std::ofstream::out);
        // for (int i = 0; i < size * size; i++)
        // {
        //     ofs_2 << " " << p_matrix[i];
    ...
    printf("A 2*2 sub matrix from parallel version\n");
    ...
    printf("\n");
    printf("A 2*2 sub matrix from serial version\n");
    for(int i=sr_sm;i<sr_sm+2;i++){
        for(int j=sc_sm;j<sc_sm+2;j++){
